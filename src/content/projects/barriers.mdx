...
<<<<<<< Updated upstream
Waiting threads enter a hot loop like `while(!release);` or `while(sense != localsense)` to continuous check when it can leave the barrier.
=======
Waiting threads enter a hot loop like `while(!release);` or `while(sense != localsense)` to continuous check when they can leave the barrier.
>>>>>>> Stashed changes
